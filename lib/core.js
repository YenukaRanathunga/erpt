...
export function performAction(state, user, action, payload = {}) {
  if (action === 'request:create') {
    ...
    state.requests.push({
      ...
      status:'pending', note, printCount:0
    });
    return state;
  }
  ...
  if (action === 'toner:add') {
    ...
    const assignedUser = action === 'stock:issue' && Number(payload.userId)
      ? findOrThrow(state.users, payload.userId, 'Assigned user')
      : null;
    ...
    state.stockHistory.push({
      ...
      assignedTo:assignedUser ? assignedUser.name : (printer ? printer.location : ''),
      department:assignedUser ? assignedUser.dept : (printer ? printer.location : ''),
      source:action === 'stock:receive' ? 'Stock receipt' : 'Manual issue'
    });
    ...
    request.printCount=printCount;
    const requestPrinter = state.printers.find(row => row.name === request.printer);
    const requestUser = state.users.find(row => row.id === request.uid);
    if (requestPrinter) {
      if (printCount >= requestPrinter.month) requestPrinter.total += printCount - requestPrinter.month;
      requestPrinter.month = printCount;
    }
    state.stockHistory.push({
      ...
      department:requestUser ? requestUser.dept : (requestPrinter ? requestPrinter.location : ''),
      requestNo:`REQ-${String(request.id).padStart(3,'0')}`,
      printCount,
